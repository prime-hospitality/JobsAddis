...
export default function HomeScreen({ onJobSelect, onSearchPress, onBellPress, unreadCount = 0, profileName, seekerYears, pageSize, enableAnimations }: HomeScreenProps) {
  ...
  const virtualizer = useVirtualizer({
    ...
    // A two-card row measures ~390px: 183px per card plus its 12px margin.
    // Only the pre-measurement estimate, but scroll restore runs on it, so
    // keep it close.
    estimateSize: useCallback(() => 390, []),
