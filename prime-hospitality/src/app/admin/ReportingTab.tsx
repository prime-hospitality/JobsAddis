"use client";

import React, { useState, useEffect } from "react";
import { getVacancyReport, getApplicationReport, getUserGrowthReport, getPackagePerformanceReport, exportTableCsv, type ExportableTable } from "./actions";
import { runSilently } from "@/lib/silentFetch";
import { BarChart3, Download } from "lucide-react";

type Report = {
  vacancy: Awaited<ReturnType<typeof getVacancyReport>> | null;
  application: Awaited<ReturnType<typeof getApplicationReport>> | null;
  growth: Awaited<ReturnType<typeof getUserGrowthReport>> | null;
  packages: Awaited<ReturnType<typeof getPackagePerformanceReport>> | null;
};

function StatTile({ label, value, color }: { label: string; value: number | string; color: string }) {
  return (
    <div className="rounded-xl border border-[#EFF1F5] bg-[#F7F8FA]/80 p-4 flex flex-col gap-1">
      <p className="text-[10px] sm:text-xs text-[#4C5361] font-bold tracking-wider uppercase leading-snug">{label}</p>
      <p className="text-2xl font-black tracking-tight leading-none" style={{ color }}>{value}</p>
    </div>
  );
}

function DayBarChart({ data }: { data: { date: string; count: number }[] }) {
  const max = Math.max(...data.map((d) => d.count), 1);
  return (
    <div className="flex items-end gap-1 h-[130px] overflow-x-auto pb-1">
      {data.map((d) => (
        <div key={d.date} className="flex flex-col items-center gap-1" style={{ minWidth: 8 }}>
          <div
            title={`${d.count} on ${d.date}`}
            style={{ width: 8, height: `${Math.max((d.count / max) * 110, 3)}px`, background: "#1B5CBF", borderRadius: "3px 3px 0 0" }}
            className="hover:bg-indigo-400 cursor-pointer transition-colors"
          />
        </div>
      ))}
    </div>
  );
}

function HorizontalBar({ label, count, max, color }: { label: string; count: number; max: number; color: string }) {
  return (
    <div className="flex items-center gap-3 mb-2">
      <span className="text-xs text-[#6E7686] w-28 shrink-0 truncate">{label}</span>
      <div className="flex-1 bg-[#EFF1F5] rounded-full h-2.5 overflow-hidden">
        <div style={{ width: `${max > 0 ? (count / max) * 100 : 0}%`, background: color }} className="h-full rounded-full transition-all" />
      </div>
      <span className="text-xs font-bold text-[#141821] w-8 text-right shrink-0">{count}</span>
    </div>
  );
}

const EXPORT_TABLES: { key: ExportableTable; label: string; note: string }[] = [
  { key: "users", label: "Users", note: "Accounts and roles" },
  { key: "profiles", label: "Job Seekers", note: "Profiles and contact details" },
  { key: "employers", label: "Employers", note: "Companies, packages, expiry" },
  { key: "jobs", label: "Vacancies", note: "All postings and status" },
  { key: "applications", label: "Applications", note: "Who applied to what" },
];

/** On-demand CSV export of the core business tables (Agreement §17.E).
 *
 *  This is the reporting/handover convenience, NOT the disaster-recovery path.
 *  Recovery is the nightly dump in .github/workflows/backup.yml, which also
 *  captures the CV and logo files that no CSV can contain -- worth knowing,
 *  because on the Supabase Free plan that dump is the only backup there is. */
function ExportDataSection() {
  const [busy, setBusy] = useState<string | null>(null);
  const [msg, setMsg] = useState<{ text: string; ok: boolean } | null>(null);

  const download = async (table: ExportableTable, label: string) => {
    setBusy(table);
    setMsg(null);
    try {
      const res = await exportTableCsv(table);
      if (!res.success) {
        setMsg({ text: `${label} export failed: ${res.error}`, ok: false });
        return;
      }
      // Blob + object URL so the CSV never has to round-trip through a route.
      const url = URL.createObjectURL(new Blob([res.csv], { type: "text/csv;charset=utf-8;" }));
      const a = document.createElement("a");
      a.href = url;
      a.download = res.filename;
      a.click();
      URL.revokeObjectURL(url);
      setMsg({ text: `${label}: ${res.rowCount} row(s) exported.`, ok: true });
    } catch (e: unknown) {
      setMsg({ text: e instanceof Error ? e.message : "Export failed.", ok: false });
    } finally {
      setBusy(null);
    }
  };

  return (
    <div className="border-t border-[#EFF1F5] pt-6">
      <h4 className="text-sm font-bold text-[#141821] uppercase tracking-wider mb-1">Export Data</h4>
      <p className="text-xs text-[#4C5361] mb-3">
        Download a snapshot as CSV. For reporting and handover — the automatic nightly
        backup is what protects against data loss, and it also covers uploaded CVs.
      </p>
      <div className="flex flex-wrap gap-2">
        {EXPORT_TABLES.map((t) => (
          <button
            key={t.key}
            onClick={() => download(t.key, t.label)}
            disabled={busy !== null}
            title={t.note}
            className="flex items-center gap-2 rounded-lg border border-[#EFF1F5] bg-white px-3 py-2 text-xs font-semibold text-[#141821] transition-colors hover:bg-[#F7F8FA] disabled:cursor-not-allowed disabled:opacity-50"
          >
            <Download size={14} />
            {busy === t.key ? "Preparing…" : t.label}
          </button>
        ))}
      </div>
      {msg && (
        <p className={`mt-3 text-xs font-medium ${msg.ok ? "text-emerald-600" : "text-red-600"}`}>
          {msg.text}
        </p>
      )}
    </div>
  );
}

export default function ReportingTab() {
  const [days, setDays] = useState(30);
  const [report, setReport] = useState<Report>({ vacancy: null, application: null, growth: null, packages: null });
  const [loading, setLoading] = useState(true);

  const load = async (d: number) => {
    setLoading(true);
    try {
      // Wrap in runSilently so this doesn't trip the global full-screen overlay
      // on reload — this tab renders its own "Loading reports…" state.
      const [vacancy, application, growth, packages] = await runSilently(() =>
        Promise.all([
          getVacancyReport(d),
          getApplicationReport(d),
          getUserGrowthReport(d),
          getPackagePerformanceReport(),
        ])
      );
      setReport({ vacancy, application, growth, packages });
    } catch (e) {
      console.error(e);
    }
    setLoading(false);
  };

  useEffect(() => {
    load(days);
  }, [days]);

  if (loading && !report.vacancy) {
    return <div className="p-8 text-center text-[#4C5361]">Loading reports...</div>;
  }

  const maxCategory = Math.max(...(report.vacancy?.byCategory.map((c) => c.count) || [1]), 1);
  const maxPackage = Math.max(...(report.packages?.map((p) => p.activeSubscriptions) || [1]), 1);

  return (
    <div className="bg-white rounded-2xl border border-[#E2E5EC] shadow-sm overflow-hidden flex flex-col h-full">
      <div className="p-6 overflow-y-auto" style={{ maxHeight: "calc(100vh - 200px)" }}>
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            <BarChart3 size={20} className="text-[#1B5CBF]" />
            <h3 className="text-lg font-bold text-black">Reporting & Analytics</h3>
          </div>
          <select
            value={days}
            onChange={(e) => setDays(Number(e.target.value))}
            className="px-3 py-2 rounded-lg border border-[#E2E5EC] text-sm font-medium bg-white"
          >
            <option value={7}>Last 7 days</option>
            <option value={30}>Last 30 days</option>
            <option value={90}>Last 90 days</option>
          </select>
        </div>

        {/* Stat tiles */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
          <StatTile label="New Job Posts" value={report.vacancy?.totalJobs ?? 0} color="#1B5CBF" />
          <StatTile label="Applications" value={report.application?.totalApplications ?? 0} color="#12A150" />
          <StatTile label="Avg Apps / Job" value={report.application?.averageApplicationsPerJob ?? 0} color="#1B5CBF" />
          <StatTile label="New Signups" value={report.growth?.totalSignups ?? 0} color="#141821" />
        </div>

        {/* User growth */}
        <div className="mb-8">
          <h4 className="text-sm font-bold text-[#141821] uppercase tracking-wider mb-1">User Growth</h4>
          <p className="text-xs text-[#4C5361] mb-3">
            {report.growth?.jobSeekerSignups ?? 0} job seekers, {report.growth?.employerSignups ?? 0} employers signed up in the last {days} days.
          </p>
          <DayBarChart data={report.growth?.signupsPerDay || []} />
        </div>

        {/* Application volume */}
        <div className="mb-8">
          <h4 className="text-sm font-bold text-[#141821] uppercase tracking-wider mb-3">Applications Per Day</h4>
          <DayBarChart data={report.application?.applicationsPerDay || []} />
        </div>

        {/* Vacancy by category */}
        <div className="mb-8">
          <h4 className="text-sm font-bold text-[#141821] uppercase tracking-wider mb-3">Job Posts by Category</h4>
          {(report.vacancy?.byCategory || []).length === 0 ? (
            <p className="text-sm text-[#4C5361]">No job posts in this period.</p>
          ) : (
            report.vacancy?.byCategory.map((c) => (
              <HorizontalBar key={c.category} label={c.category} count={c.count} max={maxCategory} color="#1B5CBF" />
            ))
          )}
        </div>

        {/* Package performance */}
        <div>
          <h4 className="text-sm font-bold text-[#141821] uppercase tracking-wider mb-1">Package Performance</h4>
          <p className="text-xs text-[#4C5361] mb-3">Currently active subscriptions per package (snapshot, not lifetime revenue).</p>
          {(report.packages || []).length === 0 ? (
            <p className="text-sm text-[#4C5361]">No packages configured.</p>
          ) : (
            report.packages?.map((p) => (
              <div key={p.packageId} className="flex items-center justify-between mb-2">
                <HorizontalBar label={p.name} count={p.activeSubscriptions} max={maxPackage} color="#1B5CBF" />
              </div>
            ))
          )}
        </div>

        <ExportDataSection />
      </div>
    </div>
  );
}
