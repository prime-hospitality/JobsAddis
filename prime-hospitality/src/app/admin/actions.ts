...
export async function repostJob(jobId: string, newDeadline: string) {
  ...
  const { error } = await supabase
    ...
    .update({
      deadline: endOfAddisDay(newDeadline),
      status: "active",
      scheduled_at: null,
      pre_approved: false,
...
export async function createPlatformJob(form: VacancyFormState): Promise<{ success: true } | { success: false; error: string }> {
  ...
  const { data: inserted, error } = await supabase.from("jobs").insert({
    ...
    deadline: endOfAddisDay(form.deadline || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split("T")[0]),
...
export async function postJobFromTemplate(templateId: string) {
  ...
  const { error: jobErr } = await supabase.from("jobs").insert({
    ...
    deadline: endOfAddisDay(tpl.deadline || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split("T")[0]),
...
export async function updatePlatformJob(jobId: string, form: VacancyFormState, scheduledAt?: string) {
  ...
  const update: Record<string, unknown> = {
    ...
    min_years_experience: form.min_years_experience,
    gender_preference: coerceGender(form.gender_preference),
    deadline: endOfAddisDay(form.deadline),
...
export type PfeEmployer = {
  ...
  /** Full precision -- the deadline's closing instant is clamped to this, since
   *  the date-only field above rounds down past the plan's real end. */
...
function toPfeEmployer(row: any, postedToday: number): PfeEmployer {
  ...
  return {
    ...
    packageExpiresAtRaw: row.package_expires_at ?? null,
    dailyPostLimit,
    postedToday,
 ...
 *  side, including the Addis end-of-day instant and the second clamp against
 *  the plan's real expiry -- see the comment there for why both are needed. */
function resolvePfeDeadline(
  ...
  const deadlineIso = endOfAddisDay(chosen);
...
export async function createJobForEmployer(
  ...
  const { data: inserted, error } = await supabase
    ...
    .insert({
      ...
      gender_preference: coerceGender(form.gender_preference),
      deadline: resolvePfeDeadline(form.deadline, employer.packageExpiresAt, employer.packageExpiresAtRaw),
      quantity: form.quantity || 1,
...
export async function updateJobForEmployer(
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      min_years_experience: coerceYears(form.min_years_experience),
      gender_preference: coerceGender(form.gender_preference),
      deadline: resolvePfeDeadline(form.deadline, maxDeadline, employer?.package_expires_at ?? null),
      quantity: form.quantity || 1,
    })
    .eq("id", jobId);
