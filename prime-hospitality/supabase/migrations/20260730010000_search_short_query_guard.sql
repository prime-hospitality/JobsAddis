-- Stop very short keywords matching mid-word.
--
-- Measured after the first cut: searching "it" returned "Night shift waitress"
-- at 0.85, because "waitress" contains the letters "it" and the contains-match
-- did not care about word boundaries. Two-letter queries are common ("it",
-- "hr", "f&b"), so this is not an edge case.
--
-- Exact and prefix matching stay unrestricted, so "it" still finds "IT Officer"
-- through the prefix branch. Only the '%...%' contains-branches now require a
-- keyword long enough to be meaningful.
